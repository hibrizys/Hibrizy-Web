...
export default {
  ...
  mounted() {
    let hibrizy_tm_hidden_content = document.querySelectorAll('.hibrizy_tm_hidden_content')
    hibrizy_tm_hidden_content.forEach((element) => {
      ...
      document.querySelectorAll(`.details a`).forEach((element_) => {
        element_.addEventListener('click', () => {
          let popup_details = element_
            .closest('.list_inner')
            .getElementsByClassName('hibrizy_tm_hidden_content')[0]
